...
impl<'a> CommandInterpreter<'a> {
    ...
    fn interpret_get(&mut self, get: Get) -> InterpretResult {
        let frame = &mut self.state.state.frame;
    ...
    fn interpret_toggle(&mut self, toggle: Toggle) -> InterpretResult {
        ...
        match toggle {
            ...
            Toggle::ControlLine => {
                ...
                self.state.state.frame.canvas_mut().config.show_control_line =
                    !self.state.state.frame.canvas().config.show_control_line;
            }
        }
        Ok(None)
    }
    ...
    fn interpret_set_curve_type(&mut self, curve_type: ShapeType) -> InterpretResult {
        self.state.state.frame.sub_handle_mut(SetCurveType(curve_type))?;
        Ok(Some(Message::info(format!("Set curve type to {curve_type}"))))
    }

    fn get_curves_length(&mut self) -> InterpretResult {
        let result = self.state.state.frame.sub_handle(GetCurvesLength)?;
        Ok(Some(Message::info(format!("{result}"))))
    }

    fn get_length(&mut self, curve_id: usize) -> InterpretResult {
        let result = self.state.state.frame.sub_handle(GetLength(curve_id))?;
        Ok(Some(Message::info(format!("{result}"))))
    }

    fn get_point(&mut self, curve_id: usize, point_id: usize) -> InterpretResult {
        let result = self.state.state.frame.sub_handle(GetPointOnCurve(curve_id, point_id))?;
    ...
    fn move_point(&mut self, curve_id: usize, point_id: usize, x: f32, y: f32) -> InterpretResult {
        self.state.state.frame.sub_handle_mut(MovePointOnCurve(
            curve_id,
            point_id,
            Point::new(x, y),
        ))?;
    ...
    fn trochoid(&mut self, prop: TrochoidCurveProperties) -> InterpretResult {
        self.state.state.frame.sub_handler_mut().sub_handle_mut(SetTrochoidProperties(prop))?;
        Ok(None)
    }
    ...
    fn execute(&mut self, path: PathBuf, argument: Option<String>) -> InterpretResult {
        ...
        let task_id = self.state.state.tasks.register_task(path, argument)?;
    ...
    fn task(&mut self, task: Task) -> InterpretResult {
        match task {
            Task::List => {
                let tasks =
                    self.state.state.tasks.list_tasks().map(|task| format!("({task})")).join(", ");
                Ok(Some(Message::info(format!("Tasks: {tasks}"))))
            }
            Task::Kill { task_id } => {
                if !self.state.state.tasks.task_exists(task_id) {
                    return Ok(Some(Message::error(format!("Task {task_id} does not exist"))));
                }

                self.state.state.tasks.kill_task(task_id);
